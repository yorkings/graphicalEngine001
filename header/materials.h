...
struct material {
        ...
        virtual vec4 scatter(const Raypackets& r_in, const hit_rec& rec,vec4 active_mask,vec4& atten_r, vec4& atten_g, vec4& atten_b, Raypackets& scatt_pack)const=0;
};
...
class lambertian_simd4:public material{
    public:
        ...
        vec4 scatter(const Raypackets& in_pack, const hit_rec &rec,vec4 active_mask, vec4& atten_r, vec4& atten_g, vec4& atten_b, Raypackets &scatt_pack) const override {
            atten_r = albedo_r;
            atten_g = albedo_g;
            atten_b = albedo_b;
            ...
            return active_mask; // All rays that hit are scattered
        }
    private:
...
class metal_simd4:public material{
    public:
        ...
        vec4 scatter(const Raypackets& in_pack,const hit_rec& rec,vec4 active_mask,vec4& atten_r, vec4& atten_g, vec4& atten_b,Raypackets &scatt_pack)const override{
            ...
            vec4 valid=_mm_cmpgt_ps(scatt_dot_n, _mm_setzero_ps());  
            return _mm_and_ps(valid, active_mask);
        }
    private:
        vec4 albedo_r, albedo_g, albedo_b;
        vec4 fuzz;    
